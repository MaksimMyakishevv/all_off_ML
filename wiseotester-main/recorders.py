from model import Question, Data, TestHistory, QuestionHistory, app, db
...
def add_test_record(ttext, tsubject, test_id):
    record = QuestionHistory(title=ttext, subject=tsubject, test_id=test_id)
    db.session.add(record)
    db.session.commit()
